...
class zurcher():
    ...
    def psi(self,pk,Finv=None):
        ...
        #slide 13
        value_keep = -self.cost + eulerc - np.log(pk) #choice prob of keeping
        value_replace = -self.RC + eulerc - np.log(1-pk) #choice prob of changing
        #Vi har to mulige udfald, derfor er det samlede udfald
        pv = value_keep*pk + value_replace*(1-pk)

        self.Vsigma = np.ravel(self.Finv@pv)
        #np.ravel = flattens
        #se def af Finv ovenfor
    ...
    def lambdaa(self):
        ...
        #computer ligesom under Bellman 
